...
public class RateControllerAdvice {
	...
	public ResponseEntity<RateError> apiException( RateException e) {
		...
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new RateError(e.getMessage(),"Fail"));
	}

	@ExceptionHandler(NotFoundException.class)
	public ResponseEntity<RateError> notFoundException(NotFoundException e) {
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new RateError(e.getMessage(),"Fail"));
	}
	
	@ExceptionHandler(Exception.class)
	public ResponseEntity<RateError> generalException(Exception e) {
		
		return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new RateError("Internal server error. Please contact admin","Fail"));
	}
